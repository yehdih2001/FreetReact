...
class FreetReactKafkaResourceIT {
    ...
    void setup() {
        ...
        FreetReactKafkaResource kafkaResource = new FreetReactKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/freet-react-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/freet-react-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
